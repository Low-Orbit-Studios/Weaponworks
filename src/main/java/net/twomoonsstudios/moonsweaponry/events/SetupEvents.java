    ...
    public static void fixBowFOV(ComputeFovModifierEvent event) {
        ...
        if (player.isUsingItem()) {
            ...
            if (player.getUseItem().getItem() instanceof ModifiableBowItem bowItem) {
                ...
                event.setNewFovModifier(Minecraft.getInstance().options.fov().get()/100f * (float) (event.getFovModifier() - Math.min((0.3 * progressPercent), 0.3)));
            }
...
//                event.setNewFovModifier( Minecraft.getInstance().options.fov().get()/100f * (float) (event.getFovModifier() - Math.min(0.2 * progress, 0.2)));
//            }
        }
//        if (player.getMainHandItem().getItem() instanceof BombCannonItem bombCannonItem && bombCannonItem.isArmed(player.getMainHandItem())) {
//            event.setNewFovModifier(Minecraft.getInstance().options.fov().get()/100f * (event.getFovModifier() - 0.2F));
//        }
    }
}